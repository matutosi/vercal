...
- **【判断待ち】曜日が空の行 (単発の予定) の扱いを決める**．
  同梱の `schedule.xlsx` の 5・6 行目 (「4月10日の予定」「入学式」) がこれで，
  **雛形をそのままアップロードすると落ちる**．期間の毎日か，開始日1日だけか，を決める必要がある．
  合わせて，**開始時刻も空の「終日の予定」をどこに描くか** (メモ欄か，日付の隣か) も決める．
- **【未修正】予定名が長いと隣の日にはみ出す** (`draw_schedule` に折り返し・切り詰めがない)．
  README の出力例の画像でも「from 15:00 to 17:00」が隣の列にかかっている．
- **【リファクタ候補】** 判断は保留．
